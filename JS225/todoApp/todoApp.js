...
let todoList = function() {
  ...
  const validTodoData = (todoData) => {
    let keys = Object.keys(todoData);

    return keys.every(key => {
      return validTodoProperty(todoData, key);
    });
  };

  const validTodoProperty = (todoData, property) => {
    let invalid = !(['title', 
                     'year',
                     'month',
                     'description',
    ...
    switch (property) {
      case 'title':
        if (validTitle(todoData.title)) return true;
        break;
      case 'month':
        if (validMonth(todoData.month)) return true;
        break;
      case 'year':
        if (validYear(todoData.year)) return true;
        break;
      case 'description':
        if (validDescription(todoData.description)) return true;
        break;
      case 'completed':
  ...
  const allDataSupplied = (todoData) => {
  ...
  const validTitle = (title) => typeof title === 'string' && title.length >= 3;
