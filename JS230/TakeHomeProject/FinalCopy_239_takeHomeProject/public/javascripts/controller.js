...
class Controller {
  ...
  async init() {
    ...
    this.view.bindClickNav(this.clickNavHandler.bind(this));
  ...
  clickNavHandler(e) {
    ...
    if (current) {
      let complete = current.closest('section').id !== 'all';
  
      this.view.setActive(current.dataset.title, complete);
      this.view.renderMain();
    }
  }
  ...
  async clickTableBodyHandler(e) {
    ...
    if (e.target.tagName === 'LABEL') {
      ...
      this.view.addDataForUpdateForm(todoData);
      this.view.showModals();
