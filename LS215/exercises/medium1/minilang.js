...
Write a function that implements a miniature stack-and-register-based programming language that has the following commands (also called operations or tokens):
    ...
    register : Pop a value from the stack and divide the register value by the popped stack value, storing the integer register of the division back in the register.
    POP : Remove the topmost item from the stack and place it in the register.
    PRINT : Print the register value.

All operations are integer operations (which is only important with DIV and register).
...
Rules:
  - operations requiring two values, pops the top value from the stack and performs the operation with the value of the register, then stores the result back in the register
  - updating the stack with a register value does not remove the register value
  - arithmetic operations pop a value from the stack and update the register with the operation.
  - stack begins as empty; register starts as 0
...
P - 
Given a string of operations and values parse the string the perform the actions in the sequence, in order, updating the register and stack.

Algortihm:
declare a global stack ([]) and register (0);
create a method for each functionality.

minilang - parses the string and invokes the methods, iterates over the parsed values and uses them to invoke methods
methods - affect the register and stack as needed
*/
function emptyStackError(stack) {
  if (stack.length === 0) {
...
minilang('-3 PUSH 5 SUB PRINT POP');
// 8
